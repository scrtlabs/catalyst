...
class BcolzFiveMinuteWriterColumnMismatch(Exception):
    pass
...
def _calc_five_minute_index(market_opens, five_minutes_per_day):
    ...
    for i, market_open in enumerate(market_opens):
        ...
        start_ix = five_minutes_per_day * i
        end_ix = start_ix + five_minutes_per_day
        five_minutes[start_ix:end_ix] = five_minute_values
    return pd.to_datetime(five_minutes, utc=True, box=True)
...
class BcolzFiveMinuteBarMetadata(object):
    """
    Parameters
    ----------
    ohlc_ratio : int
    ...
    calendar :  catalyst.utils.calendars.trading_calendar.TradingCalendar
        The TradingCalendar on which the five minute bars are based.
    ...
    def read(cls, rootdir):
        ...
        with open(path) as fp:
            ...
            if version >= 1:
                five_minutes_per_day = raw_data['five_minutes_per_day']
            else:
                # version 0 always assumed crypto assets.
                five_minutes_per_day = CRYPTO_ASSETS_FIVE_MINUTES_PER_DAY
    ...
    def __init__(
        ...
        self.five_minutes_per_day = five_minutes_per_day
...
class BcolzFiveMinuteBarWriter(object):
    """
    ...
    def __init__(self,
        ...
        self._five_minutes_per_day = five_minutes_per_day
    ...
    def open(cls, rootdir, end_session=None):
        ...
        """
        metadata = BcolzFiveMinuteBarMetadata.read(rootdir)
        return BcolzFiveMinuteBarWriter(
            rootdir,
            metadata.calendar,
            metadata.start_session,
            end_session if end_session is not None else metadata.end_session,
            metadata.five_minutes_per_day,
    ...
    def last_date_in_output_for_sid(self, sid):
        ...
        num_days = data['shape'][0] // self._five_minutes_per_day
        if num_days == 0:
            # empty container
            return pd.NaT
    ...
    def write_cols(self, sid, dts, cols, invalid_data_behavior='warn'):
        ...
        if not all(len(dts) == len(cols[name]) for name in self.COL_NAMES):
            raise BcolzFiveMinuteWriterColumnMismatch(
    ...
    def truncate(self, date):
        ...
        metadata = BcolzFiveMinuteBarMetadata.read(self._rootdir)
        metadata.end_session = date
        metadata.write(self._rootdir)


class BcolzFiveMinuteBarReader(FiveMinuteBarReader):
    """
    Reader for data written by BcolzFiveMinuteBarWriter
    ...
    See Also
    --------
    catalyst.data.minute_bars.BcolzFiveMinuteBarWriter
    """
    ...
    def __init__(self, rootdir, sid_cache_size=1000):
        ...
        self._five_minutes_per_day = metadata.five_minutes_per_day
    ...
    def _get_metadata(self):
        return BcolzFiveMinuteBarMetadata.read(self._rootdir)
